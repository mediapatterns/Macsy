...
import macsy.lib.onlineLearning.PocketPerceptron;
 ...
 * The prediction and training procedure of the module are based on the pocket perceptron algorithm
 * The algorithm saves the best weight vector that has occured so far and uses it iteratively based on parameters such as the number of consecutive correct classification using perceptron
 ...
 *
 */
public class OnlineClassifierPocket extends BaseModule {
    ...
    //File to store the pocket parameters. Specify the file path.
    static final String pocket_parameters_file = "";
    ...
    // object used for communicate with the database for reading and writing
    ...
    //ratio of consecutive correct classification using pocket weights
    private double run_w;
    //ratio of total training examples correctly classified by pocket weights
    private double num_ok_w;
    //# of total training examples
    private int total_tp = 0;

    public OnlineClassifierPocket(String propertiesFilename) throws Exception {
    ...
    public void runModuleCore() throws Exception {
        ...
        //read the parameters from the pocket parameters file
        BufferedReader bf = new BufferedReader(new FileReader(pocket_parameters_file));
        String line = "";
        while ((line = bf.readLine()) != null) {
            String[] temp = line.split("=");
            if (temp[0].contentEquals("run_w")) {
                run_w = Double.parseDouble(temp[1]);
            }
            if (temp[0].contentEquals("num_ok_w")) {
                num_ok_w = Double.parseDouble(temp[1]);
            }
        ...
        OnlineLearning onlineLearning =
                new PocketPerceptron(fileName);

        PocketPerceptron pocket_perceptron = new PocketPerceptron();
        ...
        while (dtToDate.before(toDay.getTime())) {
            ...
            //setting counters for pocket perceptron
            pocket_perceptron.setRun_p(0);
            pocket_perceptron.setNum_ok_p(0);
            setTotal_tp(0);
            ...
            //pocket perceptron procedure
            //# of total training examples correctly classified by perceptron weights
            int num_ok_p = pocket_perceptron.getNum_ok_p();

            //# of consecutive correct classification using perceptron weights
            int run_p = pocket_perceptron.getRun_p();
            int total_points = getTotal_tp();

           // ratio of consecutive correct classification using perceptron weights
            double run_p_ratio = (double) run_p / total_points;
            //ratio of total training examples correctly classified by perceptron weights
            double num_ok_p_ratio = (double) num_ok_p / total_points;
            //if run of correct classification is longer than run_w (run of pocket weights)
            if (run_p_ratio > run_w) {

                //if number of correctly classified training examples are bigger than num_ok_w (correctly classified training examples by pocket weights)
                if (num_ok_p_ratio > num_ok_w) {

                    run_w = run_p_ratio;
                    num_ok_w = num_ok_p_ratio;
                    FileWriter f = new FileWriter(pocket_parameters_file);

                    //write the new parameters to the pocket parameters file
                    f.write("run_w=" + run_w + "\n");
                    f.write("num_ok_w=" + num_ok_w + "\n");
                    f.close();
                    //replace and store the model
    ...
    void training(DataPoint sample,
        ...
        //increment the number of total training points
        total_tp++;
    }
    ...
    public static void main(String[] args) throws Exception {
        OnlineClassifierPocket module = new OnlineClassifierPocket("E:/OnlineClassifier/Sports/Pocket Perceptron/articleOnlineClassifierSPORTS.settings");

        module.run();
    }

    /**
     * @return the total_tp (number of total training points)
     */
    public int getTotal_tp() {
        return total_tp;
    }

    /**
     * @param total_tp to set
     */
    public void setTotal_tp(int total_tp) {
